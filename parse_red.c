...
void	ft_parse_red_if_2(char *line, t_prs *prs, char **newline)
{
	prs->i = 2;
...
{
	ft_parse_red_if_2(line, prs, newline);
	while (line[prs->i])
	{
	...
	char **newline, t_prs *prs)
{
	...
	while (line[prs->i])
	{
		ft_parse_red_else_1(line, newline, prs);
		if (line[prs->i] == '$' && line[prs->i + 1] == '?')
		{
			...
			*newline = ft_linejoin(*newline, prs->value, ft_strlen(prs->value));
			...
			ft_parse_red_else_3(line, newline, prs);
		else if (line[prs->i] == DQUT)
		{
			prs->i++;
